# project_canonical_name — A Swarm-Capable Robotics Coordination Platform in Rust

> **{{ binary_prefix }}** /ˈmɜːrmɜːr/ — {{ description }}
>
...
3. **Edge-native, not cloud-dependent.** {{ project_canonical_name }} runs entirely on a home
...
6. **Operator CLI** (`{{ crate_prefix }}`): `init`, `enroll`, `dispatch`, `status`, `tail`, `gesture-test`, `cert`.
7. **Multi-agent-ready architecture and operations**:
   ...
   - The flow is exercised end-to-end with at least one *test* second agent (could be a second Pi running a stub firmware, or even a Linux VM running `{{ crate_prefix }}-agent` with a mock McuLink) before v1.0 is declared done.
...
| `{{ crate_prefix }}-proto` | 🟢 Domain | Versioned wire format. `.proto` + generated tonic types. *Treated as a public spec.* | std, prost, tonic |
| `{{ crate_prefix }}-app` | 🔵 App | Use cases (`DispatchMission`, `EnrollAgent`, `ProcessGesture`) and ports (`AgentRegistry`, `MissionStore`, `VisionEngine`, `McuLink`, `MatterBridge`, `Clock`). Pure logic, zero I/O. | core, proto |
| `{{ crate_prefix }}-vision` | 🔴 Infra | `VisionEngine`: camera capture, ONNX inference via `tract`. | core, app |
| `{{ crate_prefix }}-mcu` | 🔴 Infra | `McuLink`: framed UART/USB-CDC protocol. | core, app |
| `{{ crate_prefix }}-grpc` | 🔴 Infra | gRPC adapters; mTLS via rustls. | core, proto, app |
| `{{ crate_prefix }}-store` | 🔴 Infra | `MissionStore` + `AgentRegistry` via `redb`. | core, app |
| `{{ crate_prefix }}-matter` | 🔴 Infra | Matter bridge via `rs-matter`. | core, app |
| `{{ crate_prefix }}-otel` | 🔴 Infra | OTel exporter; Prometheus metrics; `tracing`. | core, app |
| `{{ crate_prefix }}-policy` | 🔴 Infra | Compliance controls as types; enforces config invariants at startup. | core, app |
| `{{ crate_prefix }}-cli` | 🟡 Adapter | The `{{ crate_prefix }}` binary. | core, app, store, grpc |
| `{{ crate_prefix }}-leader` | 🟡 Adapter | The `{{ crate_prefix }}-leader` binary. | core, app, store, grpc, matter, otel, policy |
| `{{ crate_prefix }}-agent` | 🟡 Adapter | The `{{ crate_prefix }}-agent` binary. | core, app, vision, mcu, grpc, otel, policy |
...
graph TB
  subgraph G["🟢 Domain"]
    CORE[{{ crate_prefix }}-core] --- PROTO[{{ crate_prefix }}-proto]
  end
  subgraph B["🔵 App"]
    APP[{{ crate_prefix }}-app]
  end
  subgraph R["🔴 Infrastructure"]
    VIS[{{ crate_prefix }}-vision]
    MCU[{{ crate_prefix }}-mcu]
    GRPC[{{ crate_prefix }}-grpc]
    STORE[{{ crate_prefix }}-store]
    MATTER[{{ crate_prefix }}-matter]
    OTEL[{{ crate_prefix }}-otel]
    POLICY[{{ crate_prefix }}-policy]
  end
  subgraph Y["🟡 Adapters"]
    CLI[{{ crate_prefix }}-cli]
    LEAD[{{ crate_prefix }}-leader]
    AGENT[{{ crate_prefix }}-agent]
...
pub enum CommandKind {
    ...
    **v1.0 acceptance criteria includes:** a fresh maintainer follows
    ...
    │   ├── {{ crate_prefix }}-agent/
    │   ├── {{ crate_prefix }}-web/
    │   ├── {{ crate_prefix }}-firmware/
    │   └── {{ crate_prefix }}-conformance/
    ├── xtask/
    ├── docs/
    │   ├── SPEC.md             ← this file
    │   ├── PROTOCOL.md
